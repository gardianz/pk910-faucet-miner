...
4. If provider detection fails, run with `headless:false`, inspect the rendered widget, and extend `detectProvider`/`injectToken` selectors for the provider actually shown. Record which provider the faucet currently serves. If the faucet serves reCAPTCHA v3, the grecaptcha stub is applied to the live page via page.evaluate; if the page captured a grecaptcha reference before injection, you may need to inject before page load and reload.
